...
for xi in [0, 1, 2, 3, 4]:
    ...
    for si in [4, 5, 6]:
        ...
        s("execute if score temp result matches 0 if entity @s[tag=drop"+x+"] store success score temp result run execute if entity @a[distance=..1,limit=1,nbt=!{Inventory:[{Slot:"+sj+"b}]}] run replaceitem entity @a[distance=..1,limit=1] container."+sj+" "+t+" 1")
    s("")
